...
- **兼容版本**：MaiBot **1.0.0 ~ 1.0.10**（当前版本 v1.3.0）
...
- **MaiBot 1.0.10 适配（v1.3.0）**：注入路径重构，移除对 `extra_prompt` 注入的依赖，Hook 顺序从 `LATE` 调整为 `NORMAL`，新增多 key 命名兜底的消息查找方法 `_find_messages`。
...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `config_version` | str | `"1.3.0"` | 配置版本号，由插件管理，请勿手动修改 |
...
| Hook 点 | 方法 | 时机 | 行为 |
|---------|------|------|------|
| `chat.receive.after_process` | `capture_receive` | 消息接收后（EARLY） | [1/3] 捕获 session_id，在 kwargs 被替换前保存 |
| `maisaka.replyer.before_request` | `capture_before_request` | 回复请求前（EARLY） | [2/3] 捕获 session_id，逐级兜底 |
| `maisaka.replyer.before_model_request` | `capture_before_model` | 模型调用前（EARLY） | [3/3] 捕获 session_id，最后一级兜底 |
| `maisaka.replyer.before_request` | `inject_replyer_prompt` | 回复请求前（NORMAL） | 匹配和跟踪私聊会话，不注入 extra_prompt |
| `maisaka.replyer.before_model_request` | `inject_replyer_model_prompt` | 模型调用前（NORMAL） | 注入私聊增强到最终 messages |
