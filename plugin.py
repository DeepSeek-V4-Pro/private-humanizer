...
class PrivateHumanizerPlugin(MaiBotPlugin):
    ...
    def _match(self, kwargs: dict[str, Any], config: HumanizerConfig | None = None) -> MatchResult:
        ...
        if not extracted_session_id:
            captured = getattr(self, "_last_captured_session_id", "")
            if captured and captured in self._matched_sessions():
                self.ctx.logger.info("Private Humanizer: matched via captured session=%s", captured)
                profile = active_config.target_profiles[0] if active_config.target_profiles else None
                if profile is None and active_config.plugin.target_user_ids:
                    profile = TargetProfile(
                        profile_id=active_config.plugin.target_user_ids[0],
                ...
                if profile:
                    return MatchResult(True, profile=profile, reason="captured session matched",
                        platform=profile.platform, user_id=profile.user_id, session_id=captured, chat_type="private")
    ...
    # =========================================================================
    # 三级 session_id 捕获：chat.receive → before_request → before_model_request
    # 逐级兜底，应对 kwargs 被其他插件 EARLY 钩子替换后丢失 session_id 的问题
    # =========================================================================

    def _capture_sid(self, sid: str) -> None:
        if sid:
            self._last_captured_session_id = sid

    @HookHandler(
        "chat.receive.after_process",
        name="private_humanizer_capture_receive",
        description="[1/3] Capture session_id from incoming message before model request hooks fire.",
        ...
        if isinstance(message, dict):
            ...
            if isinstance(mi, dict):
                ...
                if isinstance(ac, dict):
                    self._capture_sid(str(ac.get("session_id", "") or ac.get("stream_id", "") or ""))
        return {"action": "continue"}
    ...
    async def capture_before_request(self, **kwargs):
        self._capture_sid(str(kwargs.get("session_id", "") or kwargs.get("stream_id", "") or ""))
        return {"action": "continue"}
    ...
    @HookHandler(
        ...
        description="Inject private-chat constraints into the final replyer model messages.",
        mode=HookMode.BLOCKING,
        order=HookOrder.NORMAL,
        error_policy=ErrorPolicy.SKIP,
    )
    async def inject_replyer_model_prompt(self, **kwargs):
        ...
        messages = self._inject_prompt_into_messages(raw, prompt)
        ...
        if not modified:
            self.ctx.logger.warning("Private Humanizer: inject_replyer_model_prompt no injectable target found, skipping")
            return self._continue()

        self._audit(
            {"stage": "replyer_model_prompt", "chat_id": match.session_id, "user_id": match.user_id},
        )
        return self._continue(modified)
